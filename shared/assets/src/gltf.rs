...
pub fn load_gltf_mesh(path: &Path) -> Result<CpuMesh> {
    #[cfg(target_arch = "wasm32")]
    {
        ...
        if p.contains("assets/models/ruins.gltf") {
            ...
            let bytes: &'static [u8] = include_bytes!("../../../assets/models/ruins.decompressed.gltf");
            let (doc, buffers, _images) = gltf::import_slice(bytes)
                .context("import glTF (ruins.decompressed.gltf slice)")?;
            return merge_doc_to_cpu(&doc, &buffers);
...
fn merge_doc_to_cpu(doc: &gltf::Document, buffers: &[gltf::buffer::Data]) -> Result<CpuMesh> {
    ...
    for mesh in doc.meshes() {
        for prim in mesh.primitives() {
            ...
            let pos = match reader.read_positions() { Some(it) => it.collect::<Vec<[f32; 3]>>(), None => continue };
            let nrm: Vec<[f32; 3]> = match reader.read_normals() { Some(it) => it.collect(), None => vec![[0.0,1.0,0.0]; pos.len()] };
            let start = vertices.len();
            ...
            if indices_read.is_empty() {
                ...
                for v in indices_read { let vv = start_u + v; indices.push(u16::try_from(vv).map_err(|_| anyhow!("rebased index {} exceeds u16", vv))?); }
            }
        }
    }
    if vertices.is_empty() || indices.is_empty() { bail!("no geometry found in document"); }
    Ok(CpuMesh { vertices, indices })
}
