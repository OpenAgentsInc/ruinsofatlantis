...
impl FrameGraph {
    ...
    pub fn add(mut self, p: PassSpec) -> Self { self.passes.push(p); self }
        ...
        for p in &self.passes {
            for w in p.writes {
                ...
                if p.reads.iter().any(|r| r == w) {
                    panic!("frame-graph violation in {}: reads and writes {:?}", p.label, w);
                }
            }
        }
    }
...
pub fn graph_for(enabled_ssgi: bool, enabled_ssr: bool, enabled_bloom: bool, direct_present: bool) -> FrameGraph {
    let mut g = FrameGraph::new()
        // Sky: writes SceneColor
        .add(PassSpec { label: "sky", reads: &[], writes: &[Res::SceneColor] })
        // Main: reads Depth, writes SceneColor
        .add(PassSpec { label: "main", reads: &[Res::Depth], writes: &[Res::SceneColor] });
    if !direct_present {
        // Blit SceneColor -> SceneRead for post passes that sample color
        g = g.add(PassSpec { label: "blit_scene_to_read", reads: &[Res::SceneColor], writes: &[Res::SceneRead] });
    }
    if enabled_ssr {
        // SSR: reads linear depth + SceneRead, writes SceneColor
        g = g.add(PassSpec { label: "ssr", reads: &[Res::Depth, Res::SceneRead], writes: &[Res::SceneColor] });
    }
    if enabled_ssgi {
        // SSGI: reads depth + SceneRead, writes SceneColor (additive)
        g = g.add(PassSpec { label: "ssgi", reads: &[Res::Depth, Res::SceneRead], writes: &[Res::SceneColor] });
    }
    // Post AO: reads depth, writes SceneColor
    g = g.add(PassSpec { label: "post_ao", reads: &[Res::Depth], writes: &[Res::SceneColor] });
    if enabled_bloom {
        // Bloom: reads SceneRead, writes SceneColor
        g = g.add(PassSpec { label: "bloom", reads: &[Res::SceneRead], writes: &[Res::SceneColor] });
    }
    g
}
