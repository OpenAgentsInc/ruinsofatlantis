...
use crate::gfx::{self, anim, terrain, fx::Particle};
...
impl Renderer {
    ...
    pub(crate) fn select_clip(&self, idx: usize) -> &AnimClip {
        ...
        if let Some(c) = self.skinned_cpu.animations.get(requested) { return c; }
            ...
            if self.wizard_anim_index[self.pc_index] == 0 {
                ...
                if !self.pc_cast_fired && elapsed >= self.pc_cast_time {
                    ...
                    if let Some(origin_local) = self.right_hand_world(clip, phase) {
                        let inst = self.wizard_models.get(self.pc_index).copied().unwrap_or(glam::Mat4::IDENTITY);
                        let origin_w = inst * glam::Vec4::new(origin_local.x, origin_local.y, origin_local.z, 1.0);
                        let dir_w = (inst * glam::Vec4::new(0.0, 0.0, 1.0, 0.0)).truncate().normalize_or_zero();
                        let right_w = (inst * glam::Vec4::new(1.0, 0.0, 0.0, 0.0)).truncate().normalize_or_zero();
                        let lateral = 0.20;
    ...
    pub(crate) fn update_fx(&mut self, t: f32, dt: f32) {
        ...
        if self.wizard_count > 0 {
            ...
            for i in 0..(self.wizard_count as usize) {
                if self.wizard_anim_index[i] != 0 { continue; }
        ...
        if !self.projectiles.is_empty() && !self.server.npcs.is_empty() {
            ...
            for h in &hits {
                ...
                if let Some(idx) = self.zombie_ids.iter().position(|id| *id == h.npc) {
                    ...
                    self.damage.spawn(glam::vec3(h.pos.x, hgt + 0.9, h.pos.z), h.damage);
                }
            }
        }
        ...
        while i < self.projectiles.len() {
            ...
            if kill {
                ...
                burst.push(Particle { pos: hit, vel: glam::Vec3::ZERO, age: 0.0, life: 0.12, size: 0.06, color: [1.8, 1.2, 0.4] });
                for _ in 0..10 {
                    let a = rand_unit() * std::f32::consts::TAU;
        ...
        if !burst.is_empty() { self.particles.append(&mut burst); }

        // 2.6) Collide with wizards/PC (friendly fire on)
        if !self.projectiles.is_empty() { self.collide_with_wizards(dt, 10); }
        ...
        while j < self.particles.len() {
            ...
            p.pos += p.vel * dt;
            if p.age >= p.life || (p.pos - cam).length_squared() > max_d2 { self.particles.swap_remove(j); continue; }
            j += 1;
        }

        // 4) Upload FX instances (billboard particles)
        let mut inst: Vec<ParticleInstance> = Vec::with_capacity(self.projectiles.len() * 3 + self.particles.len());
        for pr in &self.projectiles {
            ...
            // head
            inst.push(ParticleInstance { pos: [pr.pos.x, pr.pos.y, pr.pos.z], size: 0.18, color: [pr.color[0] * head_fade, pr.color[1] * head_fade, pr.color[2] * head_fade], _pad: 0.0 });
            ...
            for k in 1..=2 {
                ...
                inst.push(ParticleInstance { pos: [p.x, p.y, p.z], size: 0.13, color: [pr.color[0] * 0.8 * fade, pr.color[1] * 0.8 * fade, pr.color[2] * 0.8 * fade], _pad: 0.0 });
            }
        }
        // Impacts
        for p in &self.particles {
            ...
            inst.push(ParticleInstance { pos: [p.pos.x, p.pos.y, p.pos.z], size, color: [p.color[0] * f * 1.5, p.color[1] * f * 1.5, p.color[2] * f * 1.5], _pad: 0.0 });
        }
        if (inst.len() as u32) > self._fx_capacity { inst.truncate(self._fx_capacity as usize); }
        self.fx_count = inst.len() as u32;
        if self.fx_count > 0 { self.queue.write_buffer(&self.fx_instances, 0, bytemuck::cast_slice(&inst)); }

        // 5) If no zombies remain, retire NPC wizards from the casting loop
        if !self.any_zombies_alive() {
            for i in 0..(self.wizard_count as usize) {
                if i == self.pc_index { continue; }
                if self.wizard_anim_index[i] == 0 { self.wizard_anim_index[i] = 2; }
            }
        }
    }
        ...
        while i < self.projectiles.len() {
            ...
            for j in 0..(self.wizard_count as usize) {
                ...
                if segment_hits_circle_xz(p0, p1, center, r) {
                    ...
                    if fatal { if j == self.pc_index { self.kill_pc(); } else { self.remove_wizard_at(j); } }
                    // impact burst
                    for _ in 0..14 {
                        let a = rand_unit() * std::f32::consts::TAU;
                        let r2 = 3.5 + rand_unit() * 1.0;
                        self.particles.push(Particle { pos: p1, vel: glam::vec3(a.cos() * r2, 2.0 + rand_unit() * 1.0, a.sin() * r2), age: 0.0, life: 0.16, size: 0.02, color: [1.8, 0.8, 0.3] });
                    }
                    self.projectiles.swap_remove(i);
                    hit_someone = true;
                    break;
    ...
    pub(crate) fn spawn_firebolt(
        ...
        self.projectiles.push(gfx::fx::Projectile { pos: origin, vel: dir * speed, t_die: t + life, owner_wizard: owner, color });
    }
    ...
    pub(crate) fn root_flat_forward(&self, clip: &AnimClip, phase: f32) -> Option<glam::Vec3> {
        ...
        let mut f = z;
        f.y = 0.0;
        if f.length_squared() > 1e-6 { Some(f.normalize()) } else { None }
    }
}
...
pub(super) fn segment_hits_circle_xz(p0: glam::Vec3, p1: glam::Vec3, c: glam::Vec3, r: f32) -> bool {
