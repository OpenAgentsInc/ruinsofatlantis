...
impl Renderer {
    ...
    pub(crate) fn draw_wyvern_static(&self, rpass: &mut wgpu::RenderPass<'_>, model_m: glam::Mat4) {
        ...
        if let (Some(vb), Some(ib)) = (&self.wyvern_static_vb, &self.wyvern_static_ib) {
            ...
            rpass.set_bind_group(0, &self.globals_bg, &[]);
            rpass.set_bind_group(1, &self.shard_model_bg, &[]);
            // Pipeline layout: 0=globals, 1=model, 2=palettes(unused), 3=material
    ...
    pub(crate) fn draw_wyvern(&self, rpass: &mut wgpu::RenderPass<'_>) {
        if self.wyvern_count == 0 {
            return;
        }
        // Wyvern-only pipeline: set(0)=globals, set(1)=skin palettes, set(2)=material
        rpass.set_pipeline(&self.wyvern_pipeline);
        rpass.set_bind_group(0, &self.globals_bg, &[]);
        rpass.set_bind_group(1, &self.wyvern_palettes_bg, &[]);
        ...
        if !self.wyvern_cpu.submeshes.is_empty() {
            for (bg, start, count) in &self.wyvern_submeshes {
                rpass.set_bind_group(2, bg, &[]);
                let s = *start as u32;
                let e = s + (*count as u32);
                rpass.draw_indexed(s..e, 0, 0..self.wyvern_count);
            }
        } else {
            rpass.set_bind_group(2, &self.wyvern_mat_bg, &[]);
