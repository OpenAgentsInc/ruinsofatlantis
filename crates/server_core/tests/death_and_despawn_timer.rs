...
fn death_sets_despawn_and_removes_after_timer() {
    ...
    // Hit with a firebolt
    s.spawn_projectile_from(wid, vec3(0.0, 0.6, 0.0), vec3(0.0, 0.0, 1.0), server_core::ProjKind::Firebolt);
    ...
    for _ in 0..30 { s.step_authoritative(0.1, &[]); }
    assert!(s.ecs.get(uid).is_none(), "dead entity should despawn after timer");
}
