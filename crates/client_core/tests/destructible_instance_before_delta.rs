...
fn delta_before_instance_is_deferred_until_instance_arrives() {
    ...
    let inst = DestructibleInstance { did: 42, world_min: [-1.0, 0.0, -1.0], world_max: [1.0, 2.0, 1.0] };
    ...
    assert_eq!(repl.drain_mesh_updates().len(), 0, "delta must be deferred until instance arrives");
    ...
    let (did, chunk, entry): (u64, (u32, u32, u32), ChunkMeshEntry) = drained.into_iter().next().unwrap();
    assert_eq!(did, 42);
