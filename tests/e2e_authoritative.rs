...
fn e2e_pc_fireball_damages_wizard_and_removes_projectile() {
    ...
    s.spawn_projectile_from_pc(vec3(-1.5, 0.6, 0.0), vec3(1.0, 0.0, 0.0), ProjKind::Fireball);
    ...
    assert!(snap.projectiles.iter().all(|p| p.kind != 1), "No lingering Fireball in snapshot");
}
