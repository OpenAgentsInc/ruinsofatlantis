...
use crate::gfx::{gbuffer, hiz, util, Renderer};
...
pub fn resize_impl(r: &mut Renderer, new_size: PhysicalSize<u32>) {
    ...
    if (w, h) != (new_size.width, new_size.height) {
        log::debug!(
            ...
            new_size.width, new_size.height, r.max_dim, w, h
        );
    }
    ...
    r.scene_color = r.device.create_texture(&wgpu::TextureDescriptor {
        ...
        size: wgpu::Extent3d { width: r.config.width, height: r.config.height, depth_or_array_layers: 1 },
        mip_level_count: 1,
        sample_count: 1,
    ...
    r.scene_view = r.scene_color.create_view(&wgpu::TextureViewDescriptor::default());
    ...
    r.scene_read_view = r.scene_read.create_view(&wgpu::TextureViewDescriptor::default());
    ...
    r.present_bg = r.device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&r.scene_view) },
            wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&r._post_sampler) },
            wgpu::BindGroupEntry { binding: 2, resource: wgpu::BindingResource::TextureView(&r.depth) },
        ],
    });
    r.post_ao_bg = r.device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&r.depth) },
            wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&r._post_sampler) },
        ],
    });
    ...
    r.ssgi_scene_bg = r.device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&r.scene_read_view) },
            wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&r._post_sampler) },
        ],
    });
    // Lighting M1 resources
    r.gbuffer = Some(gbuffer::GBuffer::create(&r.device, r.config.width, r.config.height));
    r.hiz = Some(hiz::HiZPyramid::create(&r.device, r.config.width, r.config.height));
    if let Some(h) = &r.hiz {
        r.ssr_depth_bg = r.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("ssr-depth-bg"),
            layout: &r.ssr_depth_bgl,
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&h.linear_view) },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&r.point_sampler) },
            ],
        });
    }
