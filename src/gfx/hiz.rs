...
impl HiZPyramid {
    pub fn create(device: &Device, width: u32, height: u32) -> Self {
        ...
        for i in 0..mips {
            mip_views.push(tex.create_view(&wgpu::TextureViewDescriptor { base_mip_level: i, mip_level_count: Some(1), ..Default::default() }));
        }
        Self { linear_depth: tex, linear_view, mip_views, width, height }
    }
