...
fn try_extract_material(
    ...
    let Some(handle) = state.handle.as_ref() else { return; };
...
fn save_image_rgba8(img: &bevy::render::texture::Image, out: &std::path::Path) -> anyhow::Result<()> {
    use bevy::render::render_resource::TextureFormat as Tf;
    let (w, h) = (img.texture_descriptor.size.width, img.texture_descriptor.size.height);
    let (bytes, is_owned): (&[u8], bool) = match img.texture_descriptor.format {
        ...
        Tf::Bgra8Unorm | Tf::Bgra8UnormSrgb => {
            ...
            for (i, px) in img.data.chunks_exact(4).enumerate() {
                let o = i * 4;
                out[o] = px[2]; out[o+1] = px[1]; out[o+2] = px[0]; out[o+3] = px[3];
            }
