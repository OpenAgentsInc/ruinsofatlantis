...
async fn run() -> Result<()> {
    ...
    queue.write_texture(
        ...
        wgpu::TexelCopyBufferLayout {
            offset: 0,
            bytes_per_row: Some(4 * tex_cpu.width),
            rows_per_image: Some(tex_cpu.height),
        },
        tex_size,
    );
...
fn load_gltf_mesh_and_basecolor(path: &str) -> Result<(MeshCpu, TextureCpu)> {
    let (doc, buffers, images) =
        gltf::import(path).with_context(|| format!("import glTF: {}", path))?;
    let mesh = doc.meshes().next().context("no mesh in glTF")?;
    let prim = mesh
        .primitives()
        .next()
        .context("no primitive in glTF mesh")?;
    ...
    let texinfo = prim
        ...
        .pbr_metallic_roughness()
        .base_color_texture()
        .context("baseColorTexture missing in material")?;
