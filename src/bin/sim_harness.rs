...
fn main() {
    ...
    let scenario = args
        .iter()
        .skip_while(|a| a.as_str() != "--scenario")
        .nth(1);
    ...
    match scenario {
        Some(path) => {
            ...
            match scenario::load_yaml(p) {
                Ok(scn) => {
                    println!(
                        "[sim] name={} tick_ms={} seed={:?} underwater={}",
                        scn.name, scn.tick_ms, scn.seed, scn.underwater
                    );
                    ...
                    if let Ok(fb) = loader::load_spell_spec("spells/fire_bolt.json") {
                        ...
                        if let Some(dmg) = fb.damage {
                            println!("[sim] damage.type={}", dmg.damage_type);
                        }
                    }
