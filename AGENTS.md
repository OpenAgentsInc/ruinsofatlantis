...
- Telemetry: See `docs/observability/telemetry.md` for how to use logs/metrics/traces. Enable it when working on server systems or perf/observability tasks. Configure via `data/config/telemetry.toml` or env (`LOG_LEVEL`, `METRICS_ADDR`, etc.). Client dev typically uses pretty console logs only.
